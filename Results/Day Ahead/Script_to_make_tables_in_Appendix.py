...
def replace_values(df):
    ...
    d = {'[]' : 'univariate' ,
         ...
         str(dt_only + wind_cols + irr_cols + dwt1) : 'dt + wind + irrad + DWT level 1',
         str(dt_only + wind_cols + irr_cols + dwt2) : 'dt + wind + irrad + DWT level 2',
         str(dt_only + wind_cols + irr_cols + dwt3) : 'dt + wind + irrad + DWT level 3',
         str(dt_only + wind_cols + irr_cols + dwt4) : 'dt + wind + irrad + DWT level 4'
         } 
...
def import_results(file_name):
    ...
    drop_list = ['Unnamed: 0', 'STEP_FACTOR', 'OPEN_END', 'PADDING1']
...
def make_results():
    ...
    for name in find_csv_filenames(None):
        model = name.strip('.csv').strip('_da')
...
Hparam_tuning_results['wt'].to_latex('tables/' + temporal_horizon + '_Hparam_pivot_dwt.tex',
                                            multirow = True,
                                            bold_rows = False,
                                            na_rep = '-',
...
Hparam_tuning_results['wt_AF'].to_latex('tables/' + temporal_horizon + '_Hparam_pivot_dwt_AF.tex',
                                            multirow = True,
                                            bold_rows = False,
                                            na_rep = '-',
